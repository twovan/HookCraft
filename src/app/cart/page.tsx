            ...
            <div>
              {items.map((item) => (
                <div key={item.template_id} style={{
                  ...
                  {/* Info */}
                  <div style={{ flex: 1 }}>
                    <h3 style={{ fontSize: 16, fontWeight: 600, color: '#2D2D2D', margin: '0 0 8px' }}>{item.name}</h3>
                    <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                      <span style={{ padding: '4px 12px', background: '#F5E6D3', color: '#D4A574', fontSize: 11, fontWeight: 600, borderRadius: 12 }}>{item.genre}</span>
                    </div>
                  </div>
